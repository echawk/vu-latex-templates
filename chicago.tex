\documentclass[letterpaper]{turabian-researchpaper}
\usepackage[utf8]{inputenc}
\usepackage[autostyle, english = american]{csquotes}
%\MakeOuterQuote{"}
\usepackage[pass, letterpaper]{geometry}
\usepackage[american]{babel}
\usepackage[backend=biber,notes]{biblatex-chicago} % Can also include 'annotation' within the '[]'
%\usepackage{bibleref} % uncomment if you want an easy way to quote the bible
%\addbibresource{ref.bib}
\usepackage{hyperref} %Allows links to be clicked
\usepackage{wrapfig}
\usepackage{fancyhdr} % allows the setting of a custom header
\pagestyle{fancy} %fancyhdr package
\fancyhf{} %clears out prev header/footer
\rhead{\hfill \thepage
	\\ \hfill ?name?
	\\ \hfill ?class? }
\renewcommand\headrulewidth{0pt} %gets rid of the bar > sets it to 0pt

% uncomment the below line if you need an equivalent to the MLA style citation
% \newcommand{\ccparencite}[2]{(\citeauthor{#1})}

\title{?title?}
\subtitle{}
\author{?name?}
\course{?class?}
\date{\today}

\begin{document}

\maketitle

\begin{center}
	\textbf{?title?}
\end{center}

Write Here.


%\newpage
%\nocite{*} % Removing the comment will print out everything in the .bib file
%\printbibliography

\end{document}
